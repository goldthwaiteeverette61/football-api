...
public class SysUserServiceImpl implements ISysUserService, UserService {
    ...
    /**
     * 【核心新增】私有輔助方法，用於校驗支付密碼
     */
    public void verifyPayPassword(Long userId, String payPassword) {
        SysUserVo user = this.selectUserById(userId);
        if (user == null) {
            throw new ServiceException("用戶不存在");
        }
        if (user.getPayPasswordSeted() == null || user.getPayPasswordSeted() != 1) {
            throw new ServiceException("请先设定您的支付密码");
        }

        if (!BCrypt.checkpw(payPassword, user.getPayPassword())) {
    ...
    public void setPayPassword(SetPayPasswordBo bo) {
        Long userId = LoginHelper.getUserId();
        SysUserVo sysUserVo = this.selectUserById(userId);
        if (sysUserVo == null) {
            throw new ServiceException("用户不存在");
        }

        // 1. 【核心修复】使用 BCrypt.checkpw 进行登录密码校验
        if (!BCrypt.checkpw(bo.getPassword(), sysUserVo.getPassword())) {
            throw new ServiceException("登录密码校验失败");
        }

        // 2. 根据是否已设置过支付密码，执行不同逻辑
        if (sysUserVo.getPayPasswordSeted() != null && sysUserVo.getPayPasswordSeted() == 1) {
            // --- 修改支付密码 ---
            if (StringUtils.isBlank(bo.getOldPayPassword())) {
                throw new ServiceException("旧支付密码不能为空");
            }
            // 【核心修复】使用 BCrypt.checkpw 进行旧支付密码校验
            if (!BCrypt.checkpw(bo.getOldPayPassword(), sysUserVo.getPayPassword())) {
                throw new ServiceException("旧支付密码校验失败");
            }
        }

        // 3. 更新密码并保存 (加密新密码)
        this.resetUserPayPwd(userId, BCrypt.hashpw(bo.getNewPayPassword()));
    }

    /**
     * 【核心新增】根据用户ID集合，批量查询用户列表
     */
    @Override
    public List<SysUser> selectUserListByIds(Collection<Long> ids) {
        if (CollUtil.isEmpty(ids)) {
            return Collections.emptyList();
        }
        return baseMapper.selectList(new LambdaQueryWrapper<SysUser>().in(SysUser::getUserId, ids));
    }

    @Override
    public SysUser selectUserByInvitationCode(String invitationCode) {
        if (StringUtils.isBlank(invitationCode)) {
            return null;
        }
        return baseMapper.selectOne(new LambdaQueryWrapper<SysUser>()
            .apply("UPPER(invitation_code) = {0}", invitationCode.toUpperCase()));
    }
    ...
    /**
     * 【核心新增】将用户设置为代理
     * @param userId 要设置为代理的用户ID
     */
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void setUserAsAgent(Long userId) {
        // 1. 校验用户是否存在
        SysUserVo user = this.selectUserById(userId);
        if (user == null) {
            throw new ServiceException("用户不存在");
        ...
        // 2. 查询名为 "代理" 的角色
        SysRole agentRole = roleMapper.selectOne(new LambdaQueryWrapper<SysRole>().eq(SysRole::getRoleName, "代理"));
        if (agentRole == null) {
            throw new ServiceException("“代理”角色不存在，请先在系统中创建");
        }
        Long agentRoleId = agentRole.getRoleId();

        // 3. 为用户添加 "代理" 角色 (保留其原有角色)
        // 3.1 获取用户当前所有角色的ID
        Set<Long> currentRoleIds = user.getRoles().stream()
            .map(SysRoleVo::getRoleId)
            .collect(Collectors.toSet());

        // 3.2 将 "代理" 角色的ID加入集合 (Set会自动处理重复)
        currentRoleIds.add(agentRoleId);

        // 3.3 更新用户的角色列表
        insertUserRole(userId, currentRoleIds.toArray(new Long[0]), true);

        // 4. 将用户的 agentUserId 更新为他自己的 userId
        baseMapper.update(null, new LambdaUpdateWrapper<SysUser>()
            .set(SysUser::getAgentUserId, userId)
    ...
    public int resetUserPayPwd(Long userId, String payPassword) {
        return baseMapper.update(null,
            new LambdaUpdateWrapper<SysUser>()
                .set(SysUser::getPayPassword, payPassword)
                .set(SysUser::getPayPasswordSeted,1)
                .eq(SysUser::getUserId, userId));
    }
...
//            }
     ...
     */
    @Override
    public boolean registerUserFront(SysUserBo user) {
        ...
        user.setRoleIds(new Long[]{1954653947198283778l});
        insertUserRole(user, false);
        return baseMapper.insert(sysUser) > 0;
    }

    @Override
    public List<SysUserVo> selectUserList(SysUserBo sysUserBo) {
     ...
     * @param userId 用户ID
     * @param amount 扣除的金额
     * @return 操作是否成功 (true: 成功, false: 余额不足)
     */
    @Override
    public boolean deductBalance(Long userId, BigDecimal amount) {
        if (amount == null || amount.compareTo(BigDecimal.ZERO) <= 0) {
            throw new ServiceException("操作金额必须为正数");
        }
        // 调用Mapper方法，数据库层面保证原子性
        int rowsAffected = baseMapper.deductBalance(userId, amount);
    ...
    public boolean addBalance(Long userId, BigDecimal amount) {
        ...
        // 调用Mapper方法，数据库层面保证原子性
        int rowsAffected = baseMapper.addBalance(userId, amount);
        return rowsAffected > 0;
    }
}
